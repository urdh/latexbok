...
författande) är stycket. Vi kallar det \enquote{byggsten} eftersom ett stycke är
...
\paper{c5}{\newpage}% HACK(c5): sidbryt för att undvika horunge
...
\enquote{\cmd{newpage}\cmd{newpage}} då sidbrytningskommandon endast inforgar en
...
användas för att berätta för \LaTeX{} att ordet \enquote{\textsc{FORTRAN}} inte 
får brytas, medan ordet \enquote{spankulera} får brytas på de angivna 
positionerna.
...
kapitäler (\enquote{små versaler}), och \cmd{textsf} som typsätter med ett
...
\begin{table}[tbp]\begin{minipage}{\textwidth}
	...
	\begin{tabular}{lp{0.225\textwidth}p{0.375\textwidth}}
		...
		\cmd[textnormal]{textnormal\{\ldots\}}	&
			...
			{Det typsnitt som används för \enquote{vanlig} text
			 i dokumentet.} \\
		\cmd[emph]{emph\{\ldots\}}		&
			\emph{Betonad text} &
...
\paper{a4}{\clearpage}% HACK(a4): bättre flöde med ny rubrik efter tabell
...
dess \enquote{förälder} avslutas, vilket gör att de ger en tydlig trädliknande
...
vilket innebär att objektet får \enquote{flyta iväg} en bit ifrån det ställe där
...
\begin{kod}
	...
	\begin{minipage}{0.75\textwidth} % kod
			\vfil%
			\inputlatex[10][16]{\latexbokFiguredir/2/figure.tex}%
			\vfil%
	\end{minipage}
	\\ \medskip
...
\end{tabular}
	...
	\end{minipage}
	\\ \medskip
	\fbox{\begin{minipage}{0.95\textwidth} % figur
		\centering \medskip
		\begin{tabular}{l r p{6cm}}
			\toprule 
			Konstant & Värde & Kort beskrivning \\
...
{\vspace*{-0.75\baselineskip}}% HACK(a4+c5): bättre sidbryt
\subsection{Ordlistor}\label{sec:2:glossaries}
{\vspace*{-0.25\baselineskip}}% HACK(a4+c5): bättre sidbryt
\index{ordlista}
...
\index{citattecken}\label{sec:citattecken}
...
\pack{csquotes}, som gör det enkelt med nästlade citat och olika stilar i
olika språk. Detta paket och även miljöer för lite längre citat behandlas
närmre på \cpageref{sec:2:citat}}. \Cref{tab:citat} visar hur dessa
...
\enquote{tankstreck} är i själva verket mycket närmre det som i \LaTeX{} anses vara ett
...
som \LaTeX{} anser vara \enquote{ömtåliga} i situationer de inte är tänkta att
...
\paper{a4}{\nopagebreak}% HACK(a4): sidbryt inte innan kod
\begin{latexcode}
...
rubriken \enquote{Sammanfattning} (eller, om engelska valts som språk,
\foreignquote{english}{Abstract}).
...
\index{citattecken}
I \LaTeX{} används ofta \verb|``| och \verb|''| för att typsätta citattecken\footnote{Detta förklarades redan på \cpageref{sec:citattecken}.},
...
\protect\paper{a4}{\\}% HACK(a4): bättre fotnotsradbrytning
}:
...
till exempel) som är \enquote{ömtåliga} \eng{fragile}, och som därför inte kan
