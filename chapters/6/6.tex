...
\begin{document}
\chapter{Vidare läsning}\label{sec:6}
...
\subsection{Böcker och artiklar}
\index{resurser!böcker|(}
...
\begin{description}
	...
	går 
	\paper{a4}{\\}% HACK(a4): Bättre radbrytning
	även 
	\paper{c5}{\\}% HACK(c5): Bättre radbrytning
	under namnet \pack{lshort} och är
	...
	alla 
	\paper{c5}{\\}% HACK(c5): Bättre radbrytning
	\LaTeX-kommandon och en stor mängd paket. Om du ska
	skriva ett eget paket eller en egen dokumentklass, eller bara är
	intresserad av att \enquote{hacka} \LaTeX{} lite, så bör du absolut ta en titt
	...
	\enquote{manual} till \LaTeX{} man kan komma. Främst av historiskt intresse,
...
\index{TeX Stack Exchange@\TeX{} Stack Exchange}
...
som Mercurial\footnote{\url{http://mercurial.selenic.com/}%
\\% HACK(a4+c5): bättre radbrytning i fotnot
} eller
...
\begin{description}
	...
	översätter interna strängar (till exempel \enquote{Referenser} och 
	\enquote{Sammanfattning}) till det språk som önskas och gör det möjligt för
	...
	% HACK(a4): discretionary hyphen i följande rad
	\texttt{OT1}, som inte innehåller icke-angli\-kanska tecken så som
	...
	\index{typsnitt!OTF- och TTF-@{OTF- och TTF-}}
	...
	innehåller dessutom en enorm mängd typsnitt som täcker alla situationer.
...
\paper{c5}{\newpage}% HACK(c5): bätre radbrytning
...
\begin{description}
	...
	\cmd{ggg} (\(\ggg\)) och \cmd{therefore} (\(\therefore\)), samt 
...
\index{paket!bibliografi}
Eftersom \pack{biblatex} (som diskuteras i~\cref{sec:5}) har bra stöd
...
normalt genom att ange stilen när man laddar \pack{biblatex}, dvs.
...
\begin{description}
	\item[\pack{alphabetic}]
	liknar \pack{numeric}, men använder en alfabetisk etikett skapad
	utifrån författarn och årtal istället för en numerisk etikett.

	\item[\pack{authoryear}]
	motsvarar ungefär Harvard-stilen. Referenser i texten typsätts på
	formen \enquote{Författare (2001)} (eller liknande former, se~\cref{sec:5}
	för en längre diskussion) och litteraturförteckningen på motsvarande
	sätt.

	\item[\pack{biblatex-chicago}]
	implementerar Chicago-stilen enligt \textcite{Chicago10}. Den här
	stilen ska inte laddas genom \pack{biblatex}, utan laddas istället
	som ett eget paket trots att det är en \pack{biblatex}-stil i grunden:
	...
	Referenser i texten visas på formen \enquote{[1]}, och litteraturförteckningen
...
\begin{description}
	...
	\item[\pack{subfig}]
	definierar kommandon som möjliggör skapandet av \enquote{underfigurer}, det
	vill säga grupper av relaterade figurer som alla kan refereras till
	antingen som grupp (till exempel \enquote{Figur 1}) eller individuellt (till
	exempel \enquote{Figur 1a}).
	...
	sätt, så att referensen blir \enquote{figur X på nästa sida} om figuren är på
	...
	till höger eller vänster på sidan och låter övrig text \enquote{flyta} runt
	figuren.
...
\begin{description}
	...
	\item[{\pack{todonotes}}]
	låter dig infoga (synliga) \enquote{att göra}-anteckningar i ditt dokument, tillsammans
...
\index{kompilator!XeTeX@\hologo{XeTeX}}
\index{typsnitt!OTF- och TTF-@{OTF- och TTF-}}
\index{teckenkodning!UTF@\UTF *}
...
\subsection{Skripta med \hologo{LuaTeX}}
\index{kompilator!LuaTeX@\hologo{LuaTeX}}
\index{teckenkodning!UTF@\UTF *}
...
\subsection{Ett alternativ: \hologo{ConTeXt}}
\index{ConTeXt@\hologo{ConTeXt}}
...
\index{LaTeX3@\LaTeX3}
\index[packages]{expl3@\textsf  {expl3}}
